                ...
                @forelse ($verifications as $verification)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <button @click="showModal = true; frontImg = '{{ Storage::disk('uploads')->url($verification->front_image) }}'; backImg = '{{ $verification->back_image ? Storage::disk('uploads')->url($verification->back_image) : '' }}'; docType = '{{ $verification->document_type === 'nid' ? 'National ID' : 'Student ID' }}'" class="mt-1 text-sm text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 font-semibold inline-flex items-center transition-colors">
