...
new class extends Component
{
    ...
    public function save()
    {
        ...
        if ($this->logo) {
            $data['logo'] = $this->logo->store('payment_logos', 'uploads');
            
            // Delete old logo if updating
            if ($this->editId && $this->existingLogo) {
                Storage::disk('uploads')->delete($this->existingLogo);
            }
        }
    ...
    public function delete($id)
    {
        $method = PaymentMethod::findOrFail($id);
        if ($method->logo) {
            Storage::disk('uploads')->delete($method->logo);
        }
        $method->delete();
    }
};
                ...
                @forelse ($paymentMethods as $method)
                    <tr>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($method->logo)
                                <img src="{{ Storage::disk('uploads')->url($method->logo) }}" alt="{{ $method->name }}" class="h-10 w-auto rounded object-contain bg-white dark:bg-gray-200 p-1">
                            @else
