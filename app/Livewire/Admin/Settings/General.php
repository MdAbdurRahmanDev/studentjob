...
class General extends Component
{
    ...
    public function mount()
    {
        ...
        $this->phone_call = Setting::get('phone_call', '');
    }
    ...
    public function saveSettings()
    {
        $this->validate([
            ...
            'phone_call' => 'nullable|string|max:255',
        ]);
        ...
        Setting::set('phone_call', $this->phone_call);

        if ($this->site_logo) {
            if ($this->existing_logo) {
                Storage::disk('uploads')->delete($this->existing_logo);
            }
            $logoPath = $this->site_logo->store('settings', 'uploads');
        ...
        if ($this->site_favicon) {
            if ($this->existing_favicon) {
                Storage::disk('uploads')->delete($this->existing_favicon);
            }
            $faviconPath = $this->site_favicon->store('settings', 'uploads');
